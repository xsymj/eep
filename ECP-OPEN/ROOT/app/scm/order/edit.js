var api = frameElement.api,
    oper = api.data.oper,
    id=api.data.id,custParame=SYSTEM.custParame,
    typeList=custParame.typeList,
    $_form=$("#base_form"),
    addNew=false,
    type=parent.type,
    editor;
var isShowMesage = api.data.isShowMesage;
console.log(isShowMesage);

var url=rootPath+"/scm/order";
var model = avalon.define({
	...
	data:{
        ...
        x:"",
        buiness_phone: "",
        is_deleted:0,status:"",ordertype:type,remark:"",start_date:"",end_date:"",sign_date:SYSTEM.date,billsn:"自动生成",delivery_date:SYSTEM.date,
			...
			productlist:[{amount:0,amt: 0,description: "",model:"",specifications:"",pack:"",id: "",product_id: "",unit:"",brand:"",spec:"",billsn:"",packformat:"",product_name: "",model:"",purchase_price:0,quoted_price:0,
    ...
    qryHead:function(v){
        ...
        model.data.isShowMesage = isShowMesage;
    ...
    sendMessage: function () {
        $.dialog.confirm("通知供应商将像供应商发送邮件和短信，请确认是否通知？", function() {
            Public.ajaxPost(url + "/sendsms.json", {phone: model.data.contacts_phone}, function (data) {
                console.log(data);
                //model.reloadData();
                parent.Public.tips({type:2,content: "短信发送成功"});
            }, function (error) {
                parent.Public.tips({type:1,content: "短信发送失败"});
            });

            Public.ajaxPost(url+"/notifySup.json", {id: model.data.id}, function(t) {
                if (t && 200 == t.status) {
                    parent.Public.tips({type:2,content:"邮件发送成功"});
                    //model.reloadData();
                } else{
                    parent.Public.tips({type:1,content: "邮件发送失败"});
                }
            });
        });
    },
    ...
    qryCustomer:function(v){//自动完成查询客户
    	...
    	Public.ajaxPost(rootPath+"/crm/customer/dataGrid.json",{keyword:v,_sortField:"realname",_sort:"asc",type:type>1?-1:0, page: 1, rows:999},function(json){
    ...
    qryProduct:function(v,idx){//自动完成查商品
    	...
    	Public.ajaxPost(rootPath+"/scm/product/dataGrid.json",{
            keyword:v,_sortField:"name",_sort:"asc",
            customer_id: model.data.customer_id
        },function(json){
