...
var THISPAGE = {
	...
	loadGrid:function() {
		function t(val, opt, row) {
			var html_con = '<div class="operating" data-id="'
					+ row.id
					+ '"><span class="fa fa-eye mrb" title="查看"></span><span class="fa fa-edit mrb" title="修改"></span><span class="fa fa-trash-o mrb" title="删除"></span></span><span class="fa fa-plus mrb" title="添加"></span>'+(row.is_deleted==1?'<span class="fa fa-reply mrb" title="恢复"></span>':'')+'</div>';
			return html_con;
		}
		function fmtAmt(v) {
	...
	addEvent:function() {
        ...
        $(".grid-wrap").on("click", ".fa-plus", function(t) {
            t.preventDefault();
            if (Business.verifyRight("BU_DELETE")) {
                var e = $(this).parent().data("id");
                handle.plus(e);
...
var handle = {
    plus: function (e) {
        ...
        Public.ajaxPost( rootPath + "/scm/product/dataGrid.json", {
            ...
            $.dialog({
                title:'供应商商品对照',
                content:"url:" + rootPath + "/?go=scm/supplier/plus",
                data: r,
                width:850,
                height:600,
                max:true,
