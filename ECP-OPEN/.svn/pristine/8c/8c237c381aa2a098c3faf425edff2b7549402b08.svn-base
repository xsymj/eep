var url=rootPath+ "/crm/custMemorialday", gridQryUrl = url+"/dataGrid.json";
	...
	loadGrid:function() {
		...
		$("#grid").jqGrid({
			...
			colModel:[ {
				...
				name:"care_type",
				label:"纪念日类型",formatter:Public.custParamefmter,
				align:"center",
		...
		$("#btn-batchDel").click(function(e) {
			...
			if (Business.verifyRight("BU_DELETE")) {
				...
				if(t.length){
					...
					parent.Public.tips({type:2,content:"请选择需要删除的客户纪念日"});
			}
		});
		$("#btn-batchAllot").click(function(e) {
			...
			if (Business.verifyRight("BU_DELETE")) {
				...
				if(t.length){
					...
					parent.Public.tips({type:2,content:"请选择需要分配的客户纪念日"});
			}
		});
		$("#btn-batchReply").click(function(e) {
			e.preventDefault();
				...
				if(t.length){
					...
					parent.Public.tips({type:2,content:"请选择需要恢复的客户纪念日"});
		});
...
var handle = {
	...
	del:function(e) {
		$.dialog.confirm("删除的客户纪念日将不能恢复，请确认是否删除？", function() {
