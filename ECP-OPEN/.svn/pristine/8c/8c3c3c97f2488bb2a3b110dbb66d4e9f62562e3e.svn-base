...
public class StorageBill extends Model<StorageBill> {
	...
	public StorageBill findById(String id,String company_id){
		String userView=Person.tableName;
		StringBuffer sql=new StringBuffer("select t.*,h.realname as head_name,c.realname as creater_name,u.realname as updater_name,dout.name depot_name,case when cc.name is null then cc1.name else cc.name end  customer_name  from ");
		sql.append(tableName);
		sql.append(" t left join ");
		sql.append(Depot.tableName);
		sql.append(" dout on dout.id=t.depot_id left join ");
		...
		sql.append(userView);
		sql.append(" h on h.id=t.head_id ");
		sql.append(" left join crm_customer cc on t.customer_id=cc.id left join scm_order so on so.billsn=t.ordersn left join  crm_customer cc1 on cc1.id=so.customer_id   ");
	...
	public void submit(String id, String company_id, String uid,String now,String nowtime) {
		...
		if(list!=null&&list.isEmpty()==false){
			for(Record r:list){
				...
				Double balance=Stock.dao.updateStock(depot_id, product_id,type<2?amount:(0-amount));//更改差异额
				//更新结余库存
	...
	public List<Record> rptList(Map<String, Object> filter) {
		...
		String type_id = (String) filter.get("type_id");
		if (StringUtils.isNotEmpty(type_id)) {
			sql.append("and o.type=?");
			parame.add(Integer.parseInt(type_id));
		}
