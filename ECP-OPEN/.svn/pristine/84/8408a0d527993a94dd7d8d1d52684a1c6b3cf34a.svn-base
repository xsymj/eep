...
var model = avalon.define({
    ...
    qryProduct:function(v,idx){//自动完成查商品
        ...
        if(!model.data.depot_id){
            ...
            var temp ;
            if(model.salereturnOrder){
                temp = 6;
            } else if (model.saleOrder) {
                temp = 4;
            } else {
                temp = ''
            }
			Public.ajaxPost(rootPath+(model.data.type>3?"/scm/stock/dataGrid.json":"/scm/product/dataGrid.json"),{keyword:v,_sortField:"name",_sort:"asc",depot_id:model.data.depot_id, type: temp},function(json){
    ...
    chooseProduct:function(e,idx){
    	...
    	model.data.productlist[idx].brasale_pricend=e.brand;
    ...
    addRow:function(){
    	model.data.productlist.push({remark: "",product_id:'',description:"",quoted_price:0,unit:"",purchase_price:0,zhamt:0,zkl:0,tax_rate:0,tax:0,amt:"",sale_price:"",amount:"",product_name:"",brand:"",spec:"",packformat:"",model:"",billsn:""});
    },//增加商品行;
...
function jisuan(){
    ...
    model.sumAmt = new Number(model.data.amount) * new Number(model.data.sale_price);
}
