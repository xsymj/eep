...
public class OrderProduct extends Model<OrderProduct> {
	...
	public List<OrderProduct> list(String id){
		return this.find("select t.*,p.name product_name,p.unit,p.billsn,p.brand,p.model,p.spec,p.packformat,pa.name unitname from "+tableName+" t,"+Product.tableName+" p,"+Parame.tableName+" pa where p.id=t.product_id and p.unit=pa.id and t.id=? order by p.name ",id);
	}
}
