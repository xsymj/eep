...
import net.loyin.model.crm.CustomerProduct;
...
public class CustomerCtrl extends AdminBaseController<Customer> {
	...
	public void customProductSave(){
		String parm = this.getPara("data");
		JSONObject object = (JSONObject) JSONObject.parse(parm);
		String customer_id =  (String) object.get("customer_id");
		JSONArray array = object.getJSONArray("products");
		Customer.dao.delCustomer(customer_id);
		for (int i = 0; i < array.size(); i++) {
			String id = array.getJSONObject(i).getString("id");
			CustomerProduct customerProduct = new CustomerProduct();
			customerProduct.set("customer_id", customer_id);
			customerProduct.set("product_id", id);
			customerProduct.save();
		}
		this.rendJson(true,null, "操作成功！",null);
	}
	@Override
