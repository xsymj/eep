...
import net.loyin.util.FreemarkerUtil;
...
public class OrderCtrl extends AdminBaseController<Order> {
	...
	public void sendsms(){
		...
		this.rendJson(true,null, "短信发送成功");
	}
	...
	public void qryOp() {
		...
		if (m != null){
			...
			m.put("company_name",getUserMap().get("company_name"));
	...
	public void save() {
		try {
			...
			//String billsn = this.getPara("billsn");
			...
			if (StringUtils.isEmpty(id)) {
				...
				po.set("purchase_id", map.get("uid"));
				po.save();
				id=po.getStr("id");
				ldata.set("id",id);
				ldata.save();
	...
	/**通知供应商*/
	@PowerBind(code={"A2_1_E","A3_1_E"},funcName="通知供应商")
	public void notifySup(){
		try{
		this.getId();
		String company_id=this.getCompanyId();
		Order order=Order.dao.findById(id, company_id);
		...
		if (order != null){
			...
			order.put("productlistlength",productlist.size());
			order.put("company_name",getUserMap().get("company_name"));
			//order.put("address",getUserMap().get("company_addr"));
		}
		//发送邮件
		String mailaddr=order.getStr("contacts_mail");
		if(mailaddr==null||"".equals(mailaddr)){
			this.rendJson(false,null,"供货商邮箱不存在！");
			return;
		}
		MailSenderInfo mail=new MailSenderInfo();
		mail.setSubject("采购单");
		String[] addr=new String[]{mailaddr};
		mail.setToAddress(addr);
		Template template = FreemarkerUtil.getTemplate("mailTemplate.ftl",this.getRequest().getRealPath("/file"));
		template.setEncoding("utf-8");
		String content = FreeMarkerTemplateUtils.processTemplateIntoString(
				template, order);
		mail.setContent(content);
		boolean issend= mail.sendMail();
		//更新mail_status 状态
		if(issend){
		Order.dao.mailStatus(id);
		this.rendJson(true, null, "通知供应商成功！");
		}else{
			this.rendJson(false, null, "发送邮件异常！");
		}
		}catch(Exception e){
			log.error(e);
			this.rendJson(false, null, "通知供应商异常！");
		}
	}
	/**通知财务*/
	@PowerBind(code={"A2_1_E","A3_1_E"},funcName="通知财务")
	public void notifyFin(){
		...
		//更新finance_status 状态
		Order.dao.financeStatus(id);
		this.rendJson(true, null, "通知财务成功！");
		}catch(Exception e){
			log.error(e);
			this.rendJson(false, null, "通知财务异常！");
		}
	}
	/**对账单详情*/
	public void dzData(){
