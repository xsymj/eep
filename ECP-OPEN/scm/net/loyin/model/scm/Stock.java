...
public class Stock extends Model<Stock> {
	...
	public Page<Stock> pageGrid(int pageNo, int pageSize,Map<String,Object>filter){
		...
		if (StringUtils.isNotEmpty(type) &&("4".equals(type)||"6".equals(type))) {
			sql.append("and p.category_type=?");
			parame.add('1');
		}
		if(StringUtils.isEmpty(depot_id)){//按产品统计库存量
			sql.append(" group by product_id,p.name,p.category,p.unit,p.stock_warn,p.billsn,p.status,p.model,p.sale_price,p.purchase_price,p.brand,p.spec ");
		}else{
			sql.append(" group by product_id,p.name,d.name,p.category,p.unit,p.stock_warn,p.billsn,p.status,p.model,p.sale_price,p.purchase_price,p.brand,p.spec ");
		}
		...
		if(StringUtils.isEmpty(depot_id)){//按产品统计库存量
			return dao.paginate(pageNo,pageSize,"select t.product_id,sum(amount) as amount,p.name product_name,p.category,p.unit,p.stock_warn,p.billsn,p.status,p.brand,p.spec,p.model,p.sale_price,p.purchase_price",sql.toString(), parame.toArray());
		}else{
			return dao.paginate(pageNo,pageSize,"select d.name depot_name,t.product_id,sum(amount) as amount,p.name product_name,p.category,p.unit,p.stock_warn,p.billsn,p.brand,p.spec,p.status,p.model,p.sale_price,p.purchase_price",sql.toString(), parame.toArray());
		}
	}
	/**库存预警*/
	...
	public Double updateStock(String depot_id,String product_id,Double amount){
		List<Record> list=Db.find("select * from "+tableName+" where depot_id=? and product_id=? ",depot_id,product_id); //查询库存中是否有该商品
